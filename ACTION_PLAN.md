...
python -m pytest tests/ -q                          # 72 个测试
...
| # | 事项 | 状态 |
|---|---|---|
| 1 | species 组词条变动（7.24 整表替换时丢失泛化物种词；7.28-7.29 几经调整，**7.29 最终确认删除 Drosophila/insect/Panarthropoda/arthropod，species 仅保留 Bilateria**；同日 concept 组新增 single-cell atlas、single-nucleus atlas）。后果：果蝇等具体物种论文关键词通道为 0，靠 AI 语义 + 期刊 + 趋势三个外部维度兜底 | 用户已定，保持现状 |

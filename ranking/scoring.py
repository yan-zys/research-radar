...
Top 15 写入 recommendations（grade：≥9 Must Read / ≥8 Important / ≥5 Reference / <5 不入库；
同日期先清空旧记录再写入）。打印 Top15 简表。
"""
import argparse
import sys
...
def grade_of(total: float):
    """等级分档：≥9 Must Read / ≥8 Important / ≥5 Reference / <5 不入库。"""
    if total >= 9:
        return "Must Read"
    if total >= 8:
        return "Important"
...
def load_kw_config() -> dict:
    """加载 keyword_config.yaml（method/trend 维度复用其命中逻辑）。"""
...
def compute_scores(rows, weights: dict, kw_config: dict, ai_veto: float = 3.0) -> list:
    ...
    for r in rows:
        ...
        enriched.append({
            ...
            "method_raw": agg.get("methods", 0) + agg.get("tools", 0),
            "trend_raw": agg.get("concept", 0),
        })
    max_rule = max((e["rule_score"] for e in enriched), default=0) or 1
    max_method = max((e["method_raw"] for e in enriched), default=0) or 1
    ...
    for e in enriched:
        dims = {
            ...
            "method_transfer_value": e["method_raw"] / max_method * 10,
...
def main():
    ap = argparse.ArgumentParser(description="五维加权打分与每日 Top 推荐")
    ...
    conn.close()
    if not top:
        print("暂无符合条件的论文（passed_filter=1 且 total_score ≥ 5）")
        return
